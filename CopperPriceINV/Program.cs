       ...
       static String strSQLConnection = "Data Source =192.168.10.22; Initial Catalog = Price; Persist Security Info=false; User ID = sa; Password = yzf; Max Pool Size=30000;Connection Timeout=1200";//資料庫連接正式區
        static void Main(string[] args)
        {
            try
            {
                ...
                string[] strID = new string[] { "銅桿OD2.6mm/kg/庫存" };
                ...
                string[] strERPID = new string[] { "A6HA02%"};
                Double dblSettingPrice = 0;

                dblSettingPrice = GetCopperPrice("銅桿OD2.6mm/kg/庫存", "A6HA02%");    //取得銅設定價格 
                GetAsp("銅桿OD2.6mm/kg/庫存");       //取得asp參數
                ...
                //DoWorking_Insert();     //新增一筆加工費
                string strResult = "庫存銅價自動輸入成功 ";
                Mail(strResult);
            }
            catch (Exception ex)
            {
                string strResult = "庫存銅價自動輸入失敗 " + ex;
                Mail(strResult);
            }
        }
        private static Double GetCopperPrice(string strID, string strERPID)     //取得銅設定價格
        {
            Double dblSHIP = 0;           //CN-VN運費
            Double dblAvgInv = 0;       //加權庫存均價
            dblSHIP = GetCopper("SHIP"); //CN-VN運費
            dblAvgInv = GetCopperAvgInv(); 
            dblAvgInv = dblAvgInv + dblSHIP;
        ...
        private static Double GetCopper(string strCopper)     //取得銅價
        {
            ...
            string strSQL = $@"select t1.login_date_from as login_date,
	                                	               ...
	                                	               t2.shipping_RMB   as shipping_RMB
                                            from   (select login_date_from,  Avg(lme_copper)  lme,  Avg(shfe_copper) shfe
                                                        from   copper_price_detail
                                                        where  login_date_from = '{DateTime.Now.AddDays(-1).ToString("yyyy/MM")}'
            ...
            do
            {
                if (dr.HasRows)
                {
                    while (dr.Read())
                    {
                        dblSHIP =  (Double)dr["shipping_RMB"];
                    }
                }
            } while (dr.NextResult());
            conn.Close(); //關閉資料庫連接
        ...
        private static Double GetCopperAvgInv()     //取得庫存加權均價 2024/7 Peggy
        {
            Double dblINVNTD = 0;      //總庫存加權均價
            SqlConnection conn = new SqlConnection(strSQLConnection);
            conn.Open(); //開啟資料庫
            SqlCommand cmd;
            SqlDataReader drinv;
            string strSQL = $@"DECLARE @vninventory float --MVE銅庫存
	                                        Select @vninventory=SUM(INVMB.MB064)/1000
			                                        From [ERPDB].[TESTMVE1].dbo.INVMB
			                                        Where MB001='A6HA01-8888' 
			                                        Or MB001='H2AA63-0304'

                                            DECLARE @inventory int --CN銅庫存
	                                        Select @inventory=SUM(INVMB.MB064)/1000
					                                        From  [ERPDB].[MSLCN].dbo.INVMB
				                                        Where MB001 like 'A6HA01%'
                                            Select 
                                            ROUND(SUM(IIF(unicop.計算欄=1,unicop.[金額(NTD)],unicop.[採購價(NTD)]*(unicop.[數量(kg)]+unicop.庫存差額)) )
				                                            /SUM(IIF(unicop.計算欄=1,unicop.[數量(kg)],(unicop.[數量(kg)]+unicop.庫存差額))),2) 總庫存加權均價
                                            From 
                                            (
	                                            Select *,
		                                            ROW_NUMBER() Over (Partition By vcopper.計算欄 Order By vcopper.採購日期 DESC) As Sort 
	                                            From
	                                            (
	                                            SELECT
			                                            MVE.TC003 AS 採購日期
			                                            , MVE.TC002 AS 採購單號
			                                            , MVE.TC004 + ' ' + MVEA.MA002 AS 廠商名稱
			                                            , MVED.TD006 AS 規格
			                                            , MVE.TC009 AS 下單方式
			                                            , MVED.TD010 AS ERP採購單價			
			                                            , cum_convert 匯率
			                                            , ROUND(MVED.TD010* cum_convert/1000,2)  AS '採購價(NTD)'				
			                                            , MVE.TC023*1000 AS '數量(kg)'
			                                            , ROUND((MVED.TD010 * MVE.TC023) * cum_convert, 2) AS '金額(NTD)'
			                                            ,SUM(MVE.TC023*1000) over(ORDER BY MVE.TC002 DESC,MVED.TD003) 累加數量
			                                            ,@vninventory 總庫存
			                                            ,(@vninventory-SUM(MVE.TC023*1000) over(ORDER BY MVE.TC002 DESC,MVED.TD003)) 庫存差額
			                                            ,IIF((@vninventory-SUM(MVE.TC023*1000) over(ORDER BY MVE.TC002 DESC,MVED.TD003) )>0,'1','0') 計算欄
	                                            ...
	                                            WHERE (MVE.TC001 = N'M330')--單別
		                                            AND (MVED.TD004 = N'A6HA01-8888')
		                                            AND (MVE.TC003 >=  DATEADD(MONTH,-7,Getdate()) )--採購日期(取前半年)
	                                            ...
	                                            Select *,
		                                            ROW_NUMBER() Over (Partition By copper.計算欄 Order By copper.採購日期 DESC) As Sort 
	                                            From
	                                            (
	                                            SELECT 
				                                             CN.TC003 AS 採購日期
				                                            , CN.TC002 AS 採購單號
				                                            , CN.TC004 + ' ' + CNA.MA002 AS 廠商名稱
				                                            , CND.TD006 AS 規格
				                                            , CN.TC009 AS 下單方式
				                                            ...
				                                            ,IIF((@inventory-SUM(CND.TD058) over(ORDER BY CN.TC002 DESC,CND.TD003) )>0,'1','0') 計算欄
		                                            FROM [ERPDB].[MSLCN].dbo.PURTC CN
			                                            INNER JOIN [ERPDB].[MSLCN].dbo.PURTD CND
				                                            ON CN.TC001 = CND.TD001 AND CN.TC002 = CND.TD002
			                                            INNER JOIN [ERPDB].[MSLCN].dbo.PURMA CNA
				                                            ON CNA.MA001 = CN.TC004
			                                            ,CUM--連結PRICE匯率價史資料

		                                            WHERE (CN.TC001 = N'C330')--單別
			                                            AND (CND.TD004 like N'A6HA01%')--品號	
			                                            AND (CN.TC003 >= DATEADD(MONTH,-7,Getdate()))--採購日期
                                            ...
                                            Where unicop.計算欄=1
                                            Or (unicop.計算欄=0 And unicop.Sort=1)";
            cmd = conn.CreateCommand();
            cmd.CommandText = strSQL;
            drinv = cmd.ExecuteReader();
            do
            {
                if (drinv.HasRows)
                {
                    while (drinv.Read())
                    {
                        ...
                        dblINVNTD = dblINVNTD + (Double)drinv["總庫存加權均價"];
                    }
                }
            } while (drinv.NextResult());
            conn.Close(); //關閉資料庫連接
        ...
        private static void Mail(string strResult)   //發送MAIL
        {
            ...
            MyMail.Subject = "庫存銅價設定資訊";
            MyMail.Body = strResult + "\r\n" + DateTime.Now.AddMonths(-1).ToString("yyyy/MM") +"銅價為:" +asp_purprice + "\r\n"+"結束時間:" + DateTime.Now.ToString(); //設定信件內容
